	<li class="item {cycle values="odd,even"}">
		...
		<h3><a href="{$gLucene->getResult($resultNum,'url',"`$smarty.const.BIT_ROOT_URL`index.php?content_id=`$contentId`")}&amp;highlight={$smarty.request.highlight|escape:"url"}">{$gLucene->getResult($resultNum,'title')}</a>
		</h3>
